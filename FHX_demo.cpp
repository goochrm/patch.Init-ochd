...
const double GOLDEN_RATIO = 1.61803398875; //this is used to create the different LFO increment amounts
const uint16_t BASE_INCREMENT = 1;
...
//this is code from one of the Dasiy samples, we're not using it here.  Delete?
...
//Blocking SPI transmit to FHX-8GT.  'lowerByte' contains 8 channel bits sent as gates to the LS 8 outputs
//TBD add in 16 bits, expander base number and auto call to the relevant GPIO address lines, if multi expanders in use??

void SendGates8gt(uint8_t lowerByte)
{
	uint8_t spi_buffer[4];
	const uint8_t FhxAddr = 0; //address the 8GT at '0b000' i.e. one expander with its config jumpers at 0/1

	SetFhxAddr(FhxAddr);
	spi_buffer[3] = SPI_PKT_8GT;  //N.B. buffer[3] sent first and buffer[0] is last
	spi_buffer[2] = 0x00;
	spi_buffer[1] = lowerByte;
	spi_buffer[0] = 0x00;
	spi_handle.BlockingTransmit(spi_buffer, 4);
}
...
/*
	for (uint8_t i=2; i<9; i++) //noted that ES sends config on all addresses at config change, we don't seem to need to do that
	{
		SetFhxAddr(i);
		System::Delay(1);  //Time for whatever the expanders to receive this address data....
		spi_buffer[3] = SPI_PKT_8CV_CONFIG;  
...
int main(void)
{
	...
	//TBD in future try a Non-blocking scheme, note the special memory was declared as a global for non-blocking
	//dma_buffer[0] = 0x11;
	//dma_buffer[1] = 0x00;
	//dma_buffer[2] = 0xFF;
	//dma_buffer[3] = 0xFF;

	// transmit the data - nonblocking
	//spi_handle.DmaTransmit(dma_buffer, 4, NULL, NULL, NULL);

	//declaring some other vars we will need
	...
	//this segment has some demo code for the FHX-8GT but it has no function in the Ochd demo

	//blink all the FHX-8GT gates (if one is connected)
	SendGates8gt(0xFF); //FHX-8GT all gates on
	System::Delay(250);
	SendGates8gt(0x00); //FHX-8GT all gates off
	/*
	//set ch 1 gate on
	SendGates8gt(0x01); //01 = FHX-8GT ch1 gate on
	*/

	//This segment has the Ochd LFO emulation - it's demo code

    // Declare an array to store the current counter value for each LFO
	uint16_t lfoCounters[8];

    // Declare an array to store the increment amount for each LFO
 	uint16_t lfoIncrements[8];

	// Declare an array to store the 8 instantaneous output values for the LFOs
	uint16_t outputValueArray[8];

    // Initialize LFO increment amounts based on the golden ratio
    for (int i = 0; i < LFO_COUNT; ++i) 
	{
        lfoIncrements[i] = static_cast<uint16_t>(BASE_INCREMENT * std::pow(GOLDEN_RATIO, i));
    }
	...
	while(true) //infinite loop
	{
		...
		//Read Pot CV 1 - we-ll use it to adjust the LFO rates
		float cv1KnobValue = hw.GetAdcValue(CV_1);
		float lfoBaseSpeed = 1.6 + (cv1KnobValue * 1.8);
		hw.WriteCvOut(CV_OUT_2, lfoBaseSpeed);
		uint16_t value = static_cast<uint16_t>(lfoBaseSpeed);
		//System::Delay(value);

		// for the 8x LFOs, calculate their updated output values.  We want sine waves so it uses radians and a sin function

        for (int i = 0; i < LFO_COUNT; ++i) {
            // Update the counter for each LFO
            lfoCounters[i] += lfoIncrements[i] * value;

            // Convert counter to radians
            double radians = (static_cast<double>(lfoCounters[i]) / 65535.0) * 2.0 * M_PI;

            // Calculate sine value and scale/shift it
            double sineValue = std::sin(radians);
            double outputValue = (sineValue + 1.0) * 5.0;

             // Map the outputValue to uint16_t range
            outputValueArray[i] = static_cast<uint16_t>(outputValue * (65535.0 / 10.0));

            // Debugging output, can be commented out if not needed
            // std::cout << "LFO " << i << ": Counter = " << lfoCounters[i] << ", Mapped Value = " << outputValueArray[i] << std::endl;
        }
...
/*
		//Test Code is a relic from figring out some stuff about how to use ESX-8CV

		//testing voltage sweeps on different channels with various configs 0x00 and 0xFF

		//pre config - set all channels to the same scale
		fhx8cvOutputValues[0] = SCALE_ZERO_TEN;
		fhx8cvOutputValues[1] = SCALE_ZERO_TEN;
		fhx8cvOutputValues[2] = SCALE_ZERO_TEN;
		fhx8cvOutputValues[3] = SCALE_ZERO_TEN;
		...
		fhx8cvOutputValues[7] = SCALE_ZERO_TEN;
		Sendfhx8cvValues(fhx8cvOutputValues);
		//and send the config bits
		fhx8cvConfigByte = 0x00; //this should be all 8 channels unipolar ** CONFIG 00 **
		SendConfig8cv(fhx8cvConfigByte);
		System::Delay(250);

		//set ch 8 gate on at start of test 8
		SendGates8gt(0x80); //80 = FHX-8GT ch8 gate on
		System::Delay(500);

		fhx8cvConfigByte = 0x00; //this should be all 8 channels bipolar ** CONFIG 00 **
		SendConfig8cv(fhx8cvConfigByte);
		System::Delay(250);

		//send some different CV values to FHX-8CV channel 8
		for (uint16_t i=0; i<60000; i+=1000)
		{
			fhx8cvChannelNum = FHX8CV_CH8;
			fhx8cvOutputVoltage = i;
			Sendfhx8cvVolts(fhx8cvChannelNum, fhx8cvOutputVoltage);
			System::Delay(100);
		}
		...
		//set ch 8 gate off at end of test 8
		SendGates8gt(0x00); //00 = FHX-8GT all gate channels off
		System::Delay(500);

		//weird, we need to switch off this channel - otherwise it's value is sticky in the following loop...
		//fhx8cvChannelNum = FHX8CV_CH8;
		//fhx8cvOutputVoltage = 0x0000;
		//Sendfhx8cvVolts(fhx8cvChannelNum, fhx8cvOutputVoltage);
		//System::Delay(100);
*/
	}

	//this code is a relic from some early tests figuring out how to use some patch-sm stuff - it never runs as the above loop is infinite
	while(true)
	{
        /** Debounce the switch */
        button.Debounce();
